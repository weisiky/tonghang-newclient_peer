...
//		RefreshListner();//上拉刷新监听
	...
	protected void processBiz() {
		// TODO Auto-generated method stub
		if(SearchUtil.getInstance().getSearchtype().equals(Constants.TOPICBYTOPIC)){
//			SearchTask task=new SearchTask();
//			task.execute(searchaccuratetarget);		
			showToast("搜索TOPICBYTOPIC", Toast.LENGTH_SHORT, false);
		}else if(SearchUtil.getInstance().getSearchtype().equals(Constants.USERBYNIKE)){
//			SearchTask task=new SearchTask();
//			task.execute(searchaccuratetarget);		
			showToast("搜索USERBYNIKE", Toast.LENGTH_SHORT, false);
		}else if(SearchUtil.getInstance().getSearchtype().equals(Constants.TOPICBYLABEL)){
//			SearchTask task=new SearchTask();
//			task.execute(searchaccuratetarget);
			showToast("搜索TOPICBYLABEL", Toast.LENGTH_SHORT, false);
		}else if(SearchUtil.getInstance().getSearchtype().equals(Constants.USERBYLABEL)){
//			SearchTask task=new SearchTask();
//			task.execute(searchaccuratetarget);
