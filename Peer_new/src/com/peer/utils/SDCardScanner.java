...
public class SDCardScanner {
	...
	/**
	 * 获取默认路径，如果有扩展卡，则取扩展卡，如果没有，则取内存卡
	 * 
	 * @return
	 */
	public static String getExtSDCardPath() {
		...
		List<String> storelist = SDCardScanner.getExtSDCardPaths();
		if (storelist.size() == 1) {
			ExtSDCardPath = storelist.get(0);
		} else if (storelist.size() > 1) {
			ExtSDCardPath = storelist.get(1);
		}
	...
	public static String getMemoryCardPath() {
		...
		List<String> storelist = SDCardScanner.getExtSDCardPaths();
		if (storelist.size() >= 1) {
			MemoryCardPath = storelist.get(0);
		}
		return MemoryCardPath;
	}
}
